...
class PatternLockView : GridLayout {
    ...
    public var cells: ArrayList<Cell> = ArrayList()
    ...
    constructor(context: Context, rc : Int, cc :Int ) : super(context)
    {
        var ta = context.obtainStyledAttributes(null, R.styleable.PatternLockView)
        regularCellBackground = ta.getDrawable(R.styleable.PatternLockView_plv_regularCellBackground)
        regularDotColor = ta.getColor(R.styleable.PatternLockView_plv_regularDotColor, ContextCompat.getColor(context, R.color.regularColor))
        regularDotRadiusRatio = ta.getFloat(R.styleable.PatternLockView_plv_regularDotRadiusRatio, DEFAULT_RADIUS_RATIO)
        ...
        plvRowCount = rc;//ta.getInteger(R.styleable.PatternLockView_plv_rowCount, DEFAULT_ROW_COUNT)
        plvColumnCount = cc;//ta.getInteger(R.styleable.PatternLockView_plv_columnCount, DEFAULT_COLUMN_COUNT)
    ...
    private fun notifyCellSelected(cell: Cell) {
        //Log.i("ptrn.",selectedCells.size.toString())
        if(selectedCells.size!=0)
            if(selectedCells.get(selectedCells.size-1).equals(cell)) return;
